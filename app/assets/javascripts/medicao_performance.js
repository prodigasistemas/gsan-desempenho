$(function() {
  ...
  function poolingVerificaSeArquivoExiste(nomeArquivo, formato_arquivo) {
    $.ajax({
      ...
      .done(function(response) {
        if (response.success) {
          ...
          $("#" + registroHistorico.id)
            .find(".historico-situacao")
            .html('<span class="label label-success">concluido</span>');
          $("#" + registroHistorico.id)
            .find(".link-arquivo")
            .html(
              '<a href="' +
                BASE_URL +
                "/arquivo_medicao_performance_relatorios. " +
                data["formato"] +
                "?nome_arquivo=" +
                nomeArquivo +
                '">' +
      ...
      .fail(function() {
        $("#" + registroHistorico.id)
          .find(".historico-situacao")
          .html('<span class="label label-danger">falha</span>');
      })
      .always(function() {
        console.log("complete");
      });
  ...
  function linhaDaTabela(data) {
    ...
    linhaTabela +=
      "<td class='historico-responsavel'>" + data.responsavel + "</td>";
    linhaTabela += "<td class='link-arquivo'>" + data.nome_arquivo + "</td>";
    linhaTabela +=
      "<td class='historico-situacao'><span class='label label-warning'>" +
      data.situacao_arquivo +
      "</span></td>";
